...
6. **Optimize other skills for Obsidian context (recommended)**  
   ...
   - **Scan:** enumerate `.cursor/skills/**/SKILL.md` (and optionally `.cursor/agents/*.md` if they describe workflows that need project orientation).
   ...
   - **Edit pattern (minimal):** where appropriate, add a short subsection (a few lines) near the top or under “When to use”, e.g.  
     - Point to **`.cursor/skills/obsidian-project-context/SKILL.md`** and state: if `.cursor/obsidian-docs-path` is set, **read `agent-context.md`** (or `_index.md`) in that folder **before** large codebase searches for orientation — then open specific files from the docs’ “Key files” / tables as needed.
