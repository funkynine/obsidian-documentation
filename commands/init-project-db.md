...
1. Check whether `.cursor/obsidian-docs-path` exists in this repo and contains a non-empty **single line** (absolute path to the **project docs folder** inside the vault — same convention as `obsidian-project-context`, typically `<vault>/projects/<projectName>/`).
